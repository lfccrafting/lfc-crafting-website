document.addEventListener("DOMContentLoaded", async()=>{
  ...
  try{
    const {data,error}=await window.lfcSupabase.from("public_order_overview").select("*").limit(10);
    ...
    for(const r of data){
      ...
      body.insertAdjacentHTML("beforeend",`<tr><td>${escHtml(d)}</td><td>${escHtml(r["Auftragsnummer"])}</td><td>${escHtml(r["Status"])}</td><td>${escHtml(r["Zusätzliche Infos"])}</td></tr>`);
    }
